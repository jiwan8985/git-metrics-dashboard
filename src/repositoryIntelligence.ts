...
export function buildStreakCard(streak: number, longestStreak: number, repoName: string): string {
    const streakEmoji = streak >= 30 ? '🔥🔥🔥' : streak >= 14 ? '🔥🔥' : streak >= 7 ? '🔥' : '✨';
    return [
        `${streakEmoji} ${streak}-day commit streak on [${repoName}]!`,
        `Longest streak: ${longestStreak} days`,
        `Tracked with Git Metrics Dashboard for VS Code`,
        `https://marketplace.visualstudio.com/items?itemName=jiwan-dev.git-metrics-dashboard`
    ].join('\n');
}

// ─── P3: Git Wrapped Summary ──────────────────────────────────────────────

export function buildWrappedSummary(metrics: MetricsData, intelligence: RepositoryIntelligence, days: number): string {
    const streak = metrics.commitStreak?.currentStreak || 0;
    const longestStreak = metrics.commitStreak?.longestStreak || 0;
    const topAuthor = metrics.authorStats?.[0];
    const topFile = metrics.fileChurnStats?.[0];
    const peakHour = Object.entries(metrics.timeAnalysis?.hourlyActivity || {})
        .sort((a, b) => b[1] - a[1])[0];
    const peakHourLabel = peakHour ? `${peakHour[0]}:00` : 'N/A';

    return [
        `🎁 Git Wrapped — Last ${days} Days`,
        `Repository: ${metrics.branchStats?.currentBranch || 'main'}`,
        '',
        `📊 By the numbers:`,
        `  • ${metrics.totalCommits} commits across ${days} days`,
        `  • ${metrics.totalFiles} files touched`,
        `  • +${(metrics.totalInsertions || 0).toLocaleString()} / -${(metrics.totalDeletions || 0).toLocaleString()} lines`,
        `  • ${metrics.totalAuthors} contributor(s)`,
        '',
        `🏆 Health Score: ${intelligence.healthScore}/100 — ${intelligence.healthLabel}`,
        streak > 0 ? `🔥 Best Streak: ${longestStreak} days (current: ${streak})` : '',
        topAuthor ? `👑 Top Contributor: ${topAuthor.name} (${topAuthor.commits} commits)` : '',
        topFile ? `🔥 Hottest File: ${shortenPath(topFile.file)} (${topFile.commits} commits)` : '',
        `⏰ Peak Coding Hour: ${peakHourLabel}`,
        '',
        '---',
        'Generated by Git Metrics Dashboard for VS Code',
        'https://marketplace.visualstudio.com/items?itemName=jiwan-dev.git-metrics-dashboard'
...
export function buildMonthlyBrief(metrics: MetricsData, intelligence: RepositoryIntelligence, days: number): string {
    const now = new Date();
    const periodLabel = days <= 31 ? 'Monthly' : days <= 62 ? 'Bi-Monthly' : 'Quarterly';
    const health = intelligence.healthScore;
    const healthEmoji = health >= 80 ? '🟢' : health >= 60 ? '🟡' : '🔴';

    const topContributors = (metrics.authorStats || [])
        .slice(0, 5)
        .map((a, i) => `| ${['🥇', '🥈', '🥉', '4th', '5th'][i]} | ${a.name} | ${a.commits} | ${a.percentage}% |`)
        .join('\n');

    const actions = intelligence.actions
        .slice(0, 5)
        .map((a, i) => `${i + 1}. **[${a.priority}]** ${a.title} — ${a.detail}`)
        .join('\n');

    const hotFiles = (metrics.fileChurnStats || []).slice(0, 5)
        .map(f => `- \`${shortenPath(f.file)}\` — ${f.commits} commits, churn: ${f.churnScore}`)
        .join('\n');

    const streakLines: string[] = metrics.commitStreak
        ? [
            `## 🔥 Commit Streak`,
            `Current: **${metrics.commitStreak.currentStreak}d** | Longest: **${metrics.commitStreak.longestStreak}d** | Activity Rate: ${metrics.commitStreak.activityRate}%`,
            ''
        ]
        : [];

    return [
        `# ${periodLabel} Engineering Brief`,
        `> Generated: ${now.toLocaleDateString()} | Branch: ${metrics.branchStats?.currentBranch || 'N/A'} | Period: Last ${days} days`,
        '',
        `## 📊 Repository Health`,
        `**${health}/100** ${healthEmoji} — ${intelligence.healthLabel}`,
        '',
        intelligence.summary,
        '',
        `## 📈 Activity Summary`,
        ...
        `| Lines Added | +${(metrics.totalInsertions || 0).toLocaleString()} |`,
        `| Lines Removed | -${(metrics.totalDeletions || 0).toLocaleString()} |`,
        '',
        ...streakLines,
        `## 👥 Top Contributors`,
        ...
        topContributors,
        '',
        ...(hotFiles ? [`## 🔥 High-Churn Files (Refactor Candidates)`, hotFiles, ''] : []),
        `## 🎯 Recommended Actions`,
        actions,
        '',
        `---`,
